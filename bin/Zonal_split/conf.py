...
class Config:
    def __init__(self): 
        ...
        window.set_title('ZonalSplit Config Settings')
    ...
    ##password
        ...
        mainbox.pack_start(hbox, True, False, 5)
        hbox.show()
        self.label5.set_size_request(90, 20)
        self.label5.show()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        if not os.path.exists(os.path.join(zonalpath, "..", "conf")):
            os.mkdir(os.path.join(zonalpath, "..", "conf"))
        configpath = os.path.join(zonalpath, "..", "conf", "ZonalSplit.ini")
    else:
        root = os.getcwd()
        configpath = os.path.join(zonalpath, "ZonalSplit.ini")
