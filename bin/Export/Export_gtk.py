...
class GUI(gtk.Window):
    ...
    ##menu bar
        ...
        spinbutton.show()
        spinbutton.set_size_request(60, -1)
        ...
        if self.checkbutton1.get_active():
            ...
            for date in all_date:
                ...
                if self.entry2.get_text().replace(" ", "") == "":
                    ...
                    outshp = self.entry2.get_text().replace(" ", "")
                ...
                cmdStr = 'pgsql2shp -f %s -h %s -u %s -p %s -P %s %s "%s"' % (outshp, host, user, port, password, dbname, sql) 
...
def get_records(fieldname, num):
    try:
        ...
        if fieldname in ["slide_id", "area", "centroid_x", "centroid_y"]:
            if num == 10:
    ...
    result.sort()
    return [result, fieldname not in ["slide_id", "area", "centroid_x", "centroid_y"]]
